...
import { _PlaceSearch } from './plugins/PlaceSearch';
import { _AutoComplete } from './plugins/AutoComplete';
import { _Geocoder } from './plugins/Geocoder';
import { _Geolocation } from './plugins/Geolocation';
...
declare global {
  ...
  namespace AMap {
     ...
     * @param {[Number,Number]} [opts.zooms=[2,30]] 图层缩放等级范围，默认 [2, 30]
     ...
     * @private
     */
    abstract class CombinePolygon extends CorePolygon {
     ...
     * @private
     */
    abstract class CombinePolyline extends CorePolyline {
      /**
       * 获取线的属性
       * @name getOptions
       * @function
    ...
    abstract class CorePolygon extends VectorOverlay {
      ...
      setSideBucketPath(context: Context, vec2Path: any, holesPath: any): number;
    }
     ...
     * @param {Boolean} [opts.alwaysRender=false] 是否主动
     * @export
     * @class CustomLayer
     * @extends {Layer}
     * @example
     ...
     */
    export class Event<T = EventType> implements Eventable {
      /**
       * @private
       */
       ...
       * @instance
       */
      off(type: T, fn: (e: any) => void, context?: any): this;
       ...
       * @instance
       */
      hasEvents(type: T, fn: (e: any) => void, context?: any): boolean;
     ...
     * @param {[Number,Number]} [opts.zooms=[2,30]] 支持的缩放级别范围，默认范围 [2-30]
    ...
    export interface IGeometryUtil {
       ...
       *
       * @param {LngLatLike} p
       * @param {LngLatLike[]} rect
       * @memberof GeometryUtil
       */
      isPointInBbox(p: LngLatLike, ring: LngLatLike[]): boolean;
      /**
       * 判断环是否在另一个环内
       *
       * @param {LngLatLike[]} ring1
     ...
     * @param {[Number, Number]} [opts.zooms=[2,20]] 地图显示的缩放级别范围, 默认为[2, 20]，取值范围[2 ~ 30]
     ...
     * ]
     * ]
     * @param {any} [opts.WebGLParams={}] 额外配置的WebGL参数 eg: preserveDrawingBuffer
     * @example
    ...
    class Map_2 extends CoreMap {
       ...
       * @param {Number} [maxZoom=zooms[1]] 最大 zoom 级别
       * @memberof Map
       * @instance
       * @example
       ...
       * @function
       * @instance
       * @param {Array<Array<number>>} maskPath 掩模范围
    ...
    export class Marker extends OverlayDOM {
      ...
      getIEVersion(): -1 | 6 | 7 | 8 | 9 | 10 | 11 | 'edge';

      watchlabelElement(): void;
    }
    ...
    class OverlayDOM extends Overlay {
      ...
      updateOverlayContexMenu(params?: any): void;
     ...
     * @param {number} [opts.fillOpacity=0.5] 多边形填充透明度，取值范围[0,1]，0表示完全透明，1表示不透明。默认为0.5
     * @param {boolean} [opts.draggable=false] 设置多边形是否可拖拽移动，默认为false
     * @param {number} [opts.extrusionHeight=0] 设置多边形是否拉伸为的多面体厚度值。默认值为0
     * @param {String[]|String} [opts.wallColor = #00D3FC] 多面体侧面颜色，支持 rgba、rgb、十六进制等。默认为#00D3FC
     * @param {String[]|String} [opts.roofColor = #00B2D5] 多面体顶面颜色，支持 rgba、rgb、十六进制等。默认为#00B2D5
     * @param {object} [opts.extData]  用户自定义属性，支持JavaScript API任意数据类型，如Polygon的id等。
     * @param {'solid'|'dashed'} [opts.strokeStyle = solid]  轮廓线样式，实线:solid，虚线:dashed
     * @param {number[]}  [opts.strokeDasharray] 勾勒形状轮廓的虚线和间隙的样式，此属性在strokeStyle 为dashed 时有效， 此属性在ie9+浏览器有效 取值：
     * 实线：[0,0,0]
    ...
    export class Polygon extends CorePolygon {
       ...
       * @instance
       */
      contains(originPoint: LngLatLike, e?: any): boolean;
      ...
      _setMouseStatus1(): void;

      _setMouseStatus2(): void;
    }
    ...
    export interface PolygonSharedOptions {
      ...
      roofColor?: string | Function;
      wallColor?: string | Function;
      extData?: any;
      strokeStyle?: 'solid' | 'dashed';
      strokeDasharray?: [number, number] | [number, number, number, number];
     ...
     * [x]、[y]、[z]分别替代切片的xyz。
     * @param {[Number,Number]} [opts.zooms=[2,30]] 支持的缩放级别范围，默认范围 [2-30]
     * @param {[Number, Number]} [opts.dataZooms=[2,30]] 数据支持的缩放级别范围，默认范围 [2-30]
     * @param {Number} [opts.opacity=1] 透明度，默认 1
     * @param {Boolean} [opts.visible=true] 是否显示，默认 true
     * @param {Number} [opts.zIndex=4] 图层叠加的顺序值，1 表示最底层。默认 zIndex：4
    ...
    export const Util: {
      ...
      lnglatToCoords(lnglat: any[], z: number): number[];
      computeTileRange(
        bounds: any,
        z: number,
        isPure?: boolean,
     ...
     * @param {number} [opts.zIndex=110] 是否显示
     * @example
    ...
    export class PlaceSearch extends _PlaceSearch {}
